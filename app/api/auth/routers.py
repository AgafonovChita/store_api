...
async def activate_account(request, user_id: int):
    ...
    if not await repo.get_repo(UserRepo).check_user_by_id(user_id=user_id):
        return response.json(
            {"message": "auth with this id is not registered"}, status=401
        )
...
async def login_user(request: Request, **kwargs):
    ...
    if (not user) or (
        not await check_password(
            password=user_data.password, hash_password=user.password
        )
    ):
        return response.json(
            {"message": "auth with this login-password is not registered"}, status=401
        )
...
async def update_token(request: Request):
    ...
    if not token:
        return response.json(
            {"status": "error", "message": "unknown token"}, status=401
        )

    access_token, _ = await create_token(
        user_id=request.ctx.user.id, type_token="access"
    )
    refresh_token, exp = await create_token(
        user_id=request.ctx.user.id, type_token="refresh"
    )
